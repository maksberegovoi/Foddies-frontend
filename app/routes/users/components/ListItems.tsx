  ...
  return (
    <div className="hidden md:grid flex-1 grid-cols-4 gap-2 lg:gap-4">
      {previews.map((src: string, i: number) => (
        <div key={i} className="aspect-square overflow-hidden rounded-xl bg-muted lg:rounded-2xl">
          <img 
            src={src} 
            ...
            <div key={itemId} className="py-[20px] first:pt-0">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex gap-2 lg:gap-3 shrink-0 pt-1">
                  ...
                  >
                    <FIcon iconName="arrow-up-right" className="size-[16px] text-foreground transition-colors group-hover:text-white lg:size-[18px]" />
                  </Link>
                  {isOwnProfile && (currentTab === "my-recipes" || currentTab === "my-favorites") && (
                    <Button 
                      onClick={() => handleDeleteItem(itemId)} 
                      disabled={deleteRecipe.isPending || removeFavorite.isPending}
                      className="group flex h-[36px] w-[36px] p-0 bg-transparent items-center justify-center rounded-full border border-border transition-all hover:bg-dark lg:h-[42px] lg:w-[42px]"
                    >
                      <FIcon iconName="trash" className="size-[18px] text-foreground transition-colors group-hover:text-white" />
                    </Button>
                  )}
                </div>
              </div>
            ...
            <div className="flex flex-col gap-0 md:flex-row md:items-center md:gap-[60px] lg:gap-[75px]">
              <div className="flex w-full items-start justify-start md:flex-1 md:items-center">
                <div className="flex items-center gap-[16px]">
                  <div 
                    className="h-[60px] w-[60px] md:h-[85px] md:w-[85px] shrink-0 overflow-hidden rounded-full cursor-pointer bg-muted"
                  ...
                  <div className="flex flex-col">
                    ...
                    {itemId !== myId && (
                      <Button
                        ...
                        className="mt-2 h-auto w-fit min-w-[130px] rounded-full px-[24px] py-[10px] text-[14px] border transition-colors bg-transparent border-border text-foreground hover:bg-dark hover:text-white"
                      >
